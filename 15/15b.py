...
cavern_pre = np.genfromtxt("input", delimiter=1, dtype=int)
prex, prey= cavern_pre.shape
cavern = np.zeros((prex*5, prey*5), dtype=int)
for i in range(5):
    for j in range(5):
        cavern[i*prex:i*prex+prex, j*prey:j*prey+prey] = (cavern_pre + i+j-1)%9 + 1
